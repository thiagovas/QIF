...
void Channel::Reset() {
  ...
  // the x variable.
  this->c_matrix_.resize(this->n_in_, std::vector<double>(this->n_out_, 0));
  this->h_matrix_.resize(this->n_in_, std::vector<double>(this->n_out_, 0));
  this->j_matrix_.resize(this->n_in_, std::vector<double>(this->n_out_, 0));
}
...
void Channel::ParseInput(std::string input_str) {
  ...
  for(unsigned i = 0; i < this->j_matrix_.size(); i++)
    for(unsigned j = 0; j < this->j_matrix_.size(); j++)
      ss >> this->j_matrix_[i][j];
  
  for(unsigned i = 0; i < this->prior_distribution_.size(); i++)
...
std::string Channel::to_string() const {
  ...
  for(unsigned i = 0; i < this->j_matrix_.size(); i++)
  {
    ...
    for(unsigned j = 1; j < this->j_matrix_[i].size(); j++)
      ss << " " << this->j_matrix_[i][j];
    ss << std::endl;
  }
  ...
  for(unsigned i = 1; i < this->prior_distribution_.size(); i++)
    ss << " " << this->prior_distribution_[i];
  ss << std::endl;
